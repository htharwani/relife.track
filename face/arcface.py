...
class ArcFaceExtractor:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.warning("hailo_platform is not installed. ArcFace running in simulation mode.")
            self.is_mock = True
            return
        ...
        try:
            ...
            if self.device_manager and self.device_manager.device is not None:
                ...
                logger.info("Using shared VDevice context in ArcFace extractor.")
            else:
                self.target = VDevice()
                logger.warning("No shared VDevice context provided. Initializing standalone VDevice for ArcFace.")
    ...
    def extract(self, face_crop):
        ...
        """
        if self.is_mock:
            return self._extract_mock(face_crop)
        ...
        try:
            ...
            logger.error(f"Error in real ArcFace inference: {e}")
            return self._extract_mock(face_crop)

    def _extract_mock(self, face_crop):
        import cv2
        try:
            # Resize to 1x1 to get the average color of the crop
            small = cv2.resize(face_crop, (1, 1))
    ...
    def __del__(self):
        ...
        if hasattr(self, 'target') and (not self.device_manager or self.device_manager.device is None):
            try:
                ...
                logger.error(f"Error releasing standalone ArcFace VDevice: {e}")
